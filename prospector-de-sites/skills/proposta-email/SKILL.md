...
- [ ] **Link como âncora HTML com texto visível limpo.** O Gmail embrulha TODO link em um redirect próprio (`google.com/url?q=...`) ao salvar — não dá pra impedir, e em corpo de texto puro o embrulho fica VISÍVEL, o que parece golpe. Por isso o rascunho é criado com corpo HTML e o link como âncora: `<a href="[URL]/proposta.html">[URL]/proposta.html</a>`, onde [URL] é a URL do lead em `publicacoes.txt` (estado `OK`) — texto visível = a URL limpa lida de lá (nunca copiada de outro e-mail nem da saída do `vercel deploy`). O redirect do Google fica só no href invisível, como em qualquer e-mail do Gmail. Depois de criar, confira o rascunho: o texto visível deve ser exatamente a URL desse lead em `publicacoes.txt`.
- [ ] **Endereço limpo e humano.** O link de cada lead vem de `publicacoes.txt` (nunca da URL que o `vercel deploy` imprime). Só o estado `OK` segue. Se a URL tiver sufixo aleatório, hash ou nome de time (ex.: `dra-ana-x7k2p-meu-time.vercel.app`) — o nome já estava ocupado na Vercel — ou o estado for `VERIFICAR`, PARE antes de enviar: link assim parece golpe e mata a confiança que a capa constrói. Mude o `prefixo` nas Configurações do dashboard (ex.: iniciais + hífen), republique o lead (vira um projeto novo) e apague o projeto antigo na Vercel. Proposta só sai com endereço apresentável.
