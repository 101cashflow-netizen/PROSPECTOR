...
- [ ] **Domínio limpo e humano.** Se o domínio do config for técnico/temporário (endereço de deploy cheio de sufixos aleatórios, tipo `sites-clientes-a1b2c3d4e5-seu-time.vercel.app`, ou qualquer subdomínio cheio de números), PARE antes de enviar qualquer proposta: link assim parece golpe e mata a confiança que a capa constrói. Oriente o usuário a usar um domínio próprio adicionado ao projeto na Vercel (Project → Settings → Domains; registro em registro.br) ou, no mínimo, o alias curto de produção `[projeto].vercel.app` com um nome de projeto apresentável, e atualizar o campo `dominio` nas Configurações do dashboard. Proposta só sai com domínio apresentável.
